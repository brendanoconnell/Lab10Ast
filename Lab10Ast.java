...
public class Lab10Ast extends Applet
{
    ...
    {
        titlePage(g,"Brendan O'Connell",6);
        /*flagOfLibya(g);
        flagOfMonaco(g);
        flagOfItaly (g);
        flagOfIreland(g);
    ...
    public static void flagOfBariya (Graphics g)
    {
        ...
        for (int d = 1; d <= numDots; d++)
        {
            ...
            if (y < 188)                       // left stripe
                Expo.setColor(g,Expo.red);

            if (y >= 189 && y < 375)           // middle stripe requires compound condition
                Expo.setColor(g,Expo.white);

            if (y >= 376 && y < 554)                      // right stripe
                Expo.setColor(g,Expo.red);
    ...
    public static void flagOfNavalSignal (Graphics g)
    {
        Expo.setBackground(g,Expo.black);
        for (int d = 1; d <= numDots; d++)
        {
